...
<?php 															
																...
																$fetchqry2 = "UPDATE `software_engineering` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<table><?php if(isset($c)) {   $fetchqry = "SELECT * FROM `software_engineering` where id='$c'  "; 
...
<?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 11){ ?>
  ...
  <tr><td><button type="submit" class="button3" name="click" >Next</button></td></tr>
  <tr><td><button type="submit" id="buttonskip" class="buttonskip" name="click" >Skip</button></td></tr>
  ...
  <tr><td><button class="button3"  name="button2" >SAVE</button></td></tr> -->
   <?php }  
    ...
    if(isset($_POST['buttonskip'])) {
      ...
      SELECT chapter_no,question FROM questions where question_no='$c'
       ";
// {    $game=" INSERT INTO skip
...
{    $game1="INSERT INTO save
 ...
 <?php if($_SESSION['clicks']>10){
	...
	$qry3 = "SELECT `ans`, `userans` FROM `software_engineering`;";
 ...
 if($no = @$_SESSION['score']<=3)
 {
     ...
     'software engineering','$no','$no1','$date','WEAK')");
     echo"WEAK SUBJECT";
 } if($no = @$_SESSION['score']>3&&$no = @$_SESSION['score']<7)
 {$skl=mysqli_query($conn,"INSERT INTO profile(name,subname,no_of_correct,no_of_incorrect,exam_time,concept) VALUES ('riya',	
   'software engineering','$no','$no1','$date','AVERAGE')");
     echo"AVERAGE SUBJECT";
 }
 if($no = @$_SESSION['score']>=7&&$no = @$_SESSION['score']<=10)
 {$skl=mysqli_query($conn,"INSERT INTO profile(name,subname,no_of_correct,no_of_incorrect,exam_time,concept) VALUES ('riya',
    'software engineering','$no','$no1','$date''STRONG')");
     echo"STRONG SUBJECT";
 }
